...
const RUDDER_ROOT = join(process.cwd(), "..");
...
function getDriveRegistry(): DriveRecord[] {
  const dbPath = join(RUDDER_ROOT, "data", "drives", "drives-database.json");
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (driveId) {
      ...
      const manifestPath = join(RUDDER_ROOT, drive.manifestFile);
