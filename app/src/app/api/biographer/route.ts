...
export async function GET() {
  try {
    const lifeStory = loadJSON("writing/life_story.json") || {};
    const lifeEvents = loadJSON("writing/life-events.json") || [];
    const voiceProfile = loadJSON("writing/voice-profile.json") || {};
...
export async function POST(request: Request) {
  try {
    const { title, content, qaPairs } = await request.json();
    ...
    if (existsSync(lifeStoryPath)) {
      try {
        const story = JSON.parse(readFileSync(lifeStoryPath, "utf-8"));
        ...
        console.error("Failed to update life_story.json stats:", err);
      }
    }

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error("[api/biographer] POST error:", error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
