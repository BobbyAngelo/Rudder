...
export default function OnboardingPage() {
  ...
  return (
    <div className="relative min-h-screen bg-background text-text-primary overflow-x-hidden flex flex-col justify-between selection:bg-emerald-500/25 selection:text-white">
      ...
      <header className="relative z-10 w-full max-w-5xl mx-auto px-6 py-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {[1, 2, 3, 4].map((i) => (
            <div 
              key={i} 
              className={`h-1.5 rounded-full transition-all duration-500 ${step === i ? "w-8 bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.4)]" : "w-1.5 bg-surface-elevated"}`}
            />
          ))}
        </div>

        <div className="text-[10px] font-mono text-emerald-400/60 uppercase">
          Local Sandbox Sync
        </div>
      </header>
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center max-w-5xl w-full mx-auto px-6 py-6">
            ...
            <div className="bg-surface/30 border border-border/60 rounded-2xl p-6 backdrop-blur-md space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-emerald-500/50 transition-all font-mono"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                {insights?.fivePeople.map((p, idx) => {
                  ...
                  return (
                    <div 
                      key={p.clusterLabel}
                      className="bg-surface/30 border border-border/60 rounded-2xl p-5 flex flex-col items-center text-center space-y-4 backdrop-blur-md group hover:border-emerald-500/25 transition-all duration-300"
                    >
                      {/* Avatar container */}
                      <div className="relative">
                        ...
                        <span className="absolute -bottom-1 -right-1 bg-emerald-400/10 text-emerald-300 border border-emerald-400/25 text-[8px] font-mono px-1.5 py-0.5 rounded-full">
                          {p.photoCount}px
                        </span>
                      </div>
                      ...
                      <div className="w-full space-y-2">
                        {hasDBName ? (
                          ...
                          <input 
                            ...
                            className="w-full bg-background/80 border border-border rounded-lg px-2.5 py-1.5 text-[11px] text-white text-center focus:outline-none focus:border-emerald-500/50 transition-all font-mono placeholder:text-text-dim"
                          />
                        )}
                          ...
                          {["friend", "colleague", "family"].map((r) => {
                            ...
                            return (
                              <button
                                ...
                                className={`text-[8px] font-mono uppercase px-1.5 py-0.5 rounded transition-all ${isSelected ? "bg-emerald-500/10 text-emerald-300 border border-emerald-400/30" : "bg-transparent text-text-dim border border-border/80 hover:text-text-secondary"}`}
                              >
                                {r.slice(0, 4)}
                              </button>
                            );
            ...
            {insights && (
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
                ...
                <div className="md:col-span-7 bg-surface/20 border border-border/60 rounded-2xl p-6 flex flex-col justify-between backdrop-blur-md relative overflow-hidden">
                  <div className="absolute right-0 bottom-0 w-64 h-64 bg-gradient-to-tr from-emerald-500/5 to-transparent blur-3xl pointer-events-none" />
                  ...
                  <div className="space-y-4 relative z-10">
                    ...
                    <div className="grid grid-cols-3 gap-3 pt-2">
                      <div className="p-3 bg-background/40 rounded-xl border border-border/40 text-left">
                        ...
                        <span className="text-[8px] font-mono text-emerald-400 font-semibold block mt-0.5">+{parseFloat((insights.quietestWeek.avgSleep - insights.bestSleepMonth.busyWeekAvgSleep).toFixed(1))} hrs</span>
                      </div>
                      ...
                      <div className="p-3 bg-background/40 rounded-xl border border-border/40 text-left">
                        ...
                        <span className="text-[8px] font-mono text-emerald-400 font-semibold block mt-0.5">-{parseFloat((insights.bestSleepMonth.busyWeekAvgRestingHR - insights.quietestWeek.avgRestingHR).toFixed(1))} bpm</span>
                      </div>
                      ...
                      <div className="p-3 bg-background/40 rounded-xl border border-border/40 text-left">
                        ...
                        <span className="text-[8px] font-mono text-emerald-400 font-semibold block mt-0.5">high vitality</span>
                      </div>
                    </div>
                  </div>

                  <div className="p-4 rounded-xl border border-emerald-500/10 bg-emerald-500/5 mt-6 text-left relative z-10 flex items-start gap-3">
            ...
            {generatedBio ? (
              <div className="space-y-6">
                ...
                <div className="bg-background border border-border/80 rounded-2xl p-8 backdrop-blur-md shadow-2xl relative">
                  ...
                  <div className="absolute top-4 right-6 flex items-center gap-1 bg-emerald-400/10 text-emerald-400 border border-emerald-400/25 px-2 py-0.5 rounded text-[8px] font-mono uppercase">
                    <Check size={9} strokeWidth={3} /> Saved to journal_entries
                  </div>

                  {/* Prose Container in beautiful Serif formatting */}
                  <div className="pt-6 font-serif text-lg md:text-xl leading-relaxed text-text-primary antialiased italic border-l-2 border-emerald-400 pl-6 my-2">
                    {generatedBio}
                  </div>
                </div>
                ...
                <div className="text-center pt-2">
                  <button 
                    ...
                    className="px-10 py-4 rounded-xl text-xs font-mono uppercase tracking-wider transition-all duration-300 hover:scale-[1.03] active:scale-[0.98] shadow-lg shadow-emerald-500/10 flex items-center gap-2 mx-auto"
                    style={{ backgroundColor: "rgb(52, 211, 153)", color: "#070709" }}
                  >
        ...
        {generatingBio && (
          <div className="max-w-md w-full space-y-6 animate-fade-in text-center py-12">
            <div className="space-y-4">
              <div className="w-12 h-12 rounded-full border-2 border-emerald-500/10 border-t-emerald-400 animate-spin mx-auto flex items-center justify-center text-lg">
                💫
              </div>
            ...
            <div className="dot-matrix rounded-xl p-5 font-mono text-[10px] leading-relaxed shadow-2xl relative text-left min-h-[160px] bg-background/80 border border-border">
              <div className="absolute top-3 right-4 flex gap-1">
                ...
                <span className="w-2 h-2 rounded-full bg-emerald-500/60 animate-ping" />
              </div>
              
              <div className="space-y-1.5 glow-text-green">
                {bioSteps.slice(0, bioStep + 1).map((stepTxt, idx) => (
                  <div key={idx} className="text-emerald-500/80 flex items-center gap-2">
                    <span className="text-emerald-400">{idx === bioStep ? "▹" : "✓"}</span>
                    {stepTxt}
                  </div>
                ))}
      ...
      <footer className="relative z-10 w-full max-w-5xl mx-auto px-6 py-4 flex items-center justify-between text-[9px] font-mono text-text-dim border-t border-border/40">
        <span>© 2026 Rudder. All credentials on-device.</span>
