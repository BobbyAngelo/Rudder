...
import {
  ...
  FolderOpen, ChevronRight, ChevronDown, Clock, Printer, Activity
} from "lucide-react";
...
export default function WritingPage() {
  ...
  const handleSave = async () => {
    ...
    if (editMode === "novel" && activeEntry) {
      try {
        ...
        metaPayload = JSON.stringify({ outline: novelOutline });
      }
    }
    ...
    try {
      await fetch("/api/writing", {
        ...
        body: JSON.stringify({
          ...
          mode,
          parent_id: parentId,
          meta_json: metaJson || activeEntry?.meta_json || "{}"
        }),
      });
      setSaveStatus("Saved");
      setDirty(false);
  ...
  // AI Actions (improve, refine, outlines, scene consistency analysis)
  const executeAiAction = async (action: "refine" | "outline" | "continue" | "summarize" | "tone" | "novel_analyze", toneVal?: string) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 truncate flex-1 min-w-0 pr-2">
                            <Icon size={11} className={active ? "text-amber-500" : "text-text-muted"} />
            ...
            >
              <div className="flex-1 min-w-0 pr-2">
                <div className="flex items-center gap-2 mb-1">
                  <Icon size={12} className={active ? "text-amber-500" : "text-text-muted"} />
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-background p-12">
            <div className="max-w-md w-full bg-surface border border-white/5 rounded-2xl p-6 text-center space-y-4">
              ...
              <p className="text-xs text-text-secondary">
                Rename the folder above or manage documents inside this branch in the files sidebar.
              </p>
              <div className="flex gap-2 pt-2">
                ...
                >
                  <Save size={12} /> Save Folder Rename
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col overflow-hidden relative" style={{ background: "var(--color-background)" }}>
                          ...
                          {interviewHistory.length === 0 && (
                            <div className="text-center py-10 space-y-2">
                              ...
                              <p className="text-[11px] px-4 leading-normal text-text-secondary">
                                I am your biographer. Speak to me to automatically record transcripts and memories.
                              </p>
                            </div>
                          )}
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                {progressPercent === 100 && <Check size={12} className="text-emerald-400 font-bold" />}
              </div>
            </div>
        ...
        <div className="flex-1 flex items-center justify-center" style={{ background: "var(--color-background)" }}>
          <div className="text-center p-8 max-w-sm">
            ...
            <p className="text-xs mb-4 text-text-muted">
              Welcome to your distraction-free writing environment. Create a new document or folder, or select an existing draft.
            </p>
