...
export default function IdentityPage() {
  ...
  return (
    <div className="page-container relative min-h-screen bg-background text-text-primary selection:bg-accent/25 selection:text-accent-text pb-16">
      
      {/* Decorative top lighting spotlights */}
      <div className="absolute top-0 left-1/4 w-[500px] h-[300px] bg-[radial-gradient(ellipse_at_top,var(--color-accent-dim),transparent_60%)] pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto px-6 pt-10 relative z-10 space-y-6">
        ...
        <div className="glass-panel rounded-2xl overflow-hidden shadow-xl border border-border bg-surface/10">
          ...
          <div className="h-28 w-full bg-gradient-to-r from-accent-dim/20 via-background to-blue-950/20 border-b border-border relative">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_50%,var(--color-accent-dim),transparent_40%)]" />
          </div>
          ...
          <div className="px-8 pb-8 relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="flex flex-col md:flex-row gap-6 items-start -mt-10">
              ...
              {/* Avatar Grid */}
              <div className="w-20 h-20 rounded-2xl border-2 border-border bg-background flex items-center justify-center text-xl font-mono font-bold text-accent shadow-xl shrink-0 ring-4 ring-accent/5 overflow-hidden">
                {editing ? (
                  "✍️"
                ) : profile?.avatar_url ? (
                ...
                {editing ? (
                  <div className="space-y-2 max-w-md">
                    <input
                      ...
                      className="bg-background border border-border rounded-lg px-3 py-1.5 text-base font-semibold text-white outline-none w-full focus:border-accent/40"
                    />
                    <input
                      value={draft.full_name || ""}
                      onChange={e => setDraft({ ...draft, full_name: e.target.value })}
                      placeholder="Full Legal Name"
                      className="bg-background border border-border rounded-lg px-3 py-1 text-xs text-text-secondary outline-none w-full focus:border-accent/40"
                    />
                  </div>
                ) : (
                  <>
                ...
                <div className="pt-2">
                  {editing ? (
                    <textarea
                      ...
                      rows={2}
                      className="bg-background border border-border rounded-lg px-3 py-2 text-xs text-text-secondary outline-none w-full focus:border-accent/40 resize-none max-w-xl"
                    />
                  ) : profile?.bio ? (
                  ...
                  {profile?.email && (
                    <div className="flex items-start justify-between group">
                      ...
                      <button 
                        ...
                        className="text-text-dim hover:text-accent p-1 opacity-0 group-hover:opacity-100 transition-all shrink-0"
                      >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {links.map((link, idx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-3.5 rounded-xl border border-border bg-surface/5 hover:border-accent/30 hover:bg-surface/20 transition-all duration-300 flex items-center justify-between group min-w-0"
                    >
                      ...
                      <ExternalLink size={12} className="text-text-dim group-hover:text-accent transition-colors shrink-0 ml-2" />
                    </a>
                  ))}
                  {links.length === 0 && (
              ...
              {editing ? (
                    ...
                    <div key={idx} className="flex gap-3 items-start p-3.5 rounded-xl border border-border bg-background">
                      ...
                      <div className="flex flex-col gap-1 items-center justify-center pt-1 shrink-0">
                        <button 
                          ...
                          className="text-text-dim hover:text-accent disabled:opacity-20 transition-colors"
                        >
                          <ArrowUp size={13} />
                        </button>
                      ...
                      <div className="flex-1 space-y-2 min-w-0">
                        <input
                          ...
                          className="w-full bg-background border border-border rounded px-2.5 py-1.5 text-xs text-white outline-none font-semibold focus:border-accent/40"
                        />
                        <textarea
                          ...
                          rows={2}
                          className="w-full bg-background border border-border rounded px-2.5 py-1.5 text-xs text-text-secondary outline-none resize-none focus:border-accent/40"
                        />
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {values.map((val, idx) => (
                    <div 
                      key={val.id || idx}
                      className="p-4 rounded-xl border border-border bg-surface/5 flex items-start gap-3.5 hover:border-accent/10 transition-colors group min-w-0"
                    >
                      <div className="w-6 h-6 rounded-lg bg-accent/10 border border-accent/25 flex items-center justify-center text-[10px] font-mono font-bold text-accent shrink-0 mt-0.5">
                        {idx + 1}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-0.5">
                          ...
                          <input
                            ...
                            className="w-full bg-background border border-border rounded px-2 py-1 text-xs text-white outline-none font-semibold focus:border-accent/40"
                          />
                        </div>
                        <div className="space-y-0.5">
                          ...
                          <input
                            ...
                            placeholder="Date"
                            className="w-full bg-background border border-border rounded px-2 py-1 text-xs text-white outline-none font-mono focus:border-accent/40"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 items-center">
                        <div className="col-span-2 space-y-0.5">
                          ...
                          <input
                            ...
                            className="w-full bg-background border border-border rounded px-2 py-1 text-xs text-text-secondary outline-none focus:border-accent/40"
                          />
                        </div>
                        <div className="space-y-0.5">
                          ...
                          <select
                            ...
                            className="w-full bg-background border border-border rounded px-2 py-1.5 text-xs text-text-secondary outline-none focus:border-accent/40"
                          >
                    ...
                    return (
                      <div key={m.id || idx} className="relative group">
                        ...
                        <div className="absolute -left-[31px] top-1.5 w-4 h-4 rounded-full border border-border bg-background flex items-center justify-center shadow">
                          <div className="w-1.5 h-1.5 rounded-full bg-accent transition-transform group-hover:scale-125" />
                        </div>
            ...
            <form onSubmit={handleSaveHabit} className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  {[
                    { hex: "var(--color-accent)", name: "Accent" },
                    ...
                    <button
                      ...
                      style={{
                        backgroundColor: color.hex.startsWith("var") ? "var(--color-accent)" : color.hex,
                        transform: habitDraft.color === color.hex ? "scale(1.15)" : "none",
                        boxShadow: habitDraft.color === color.hex ? `0 0 10px ${color.hex.startsWith("var") ? "var(--color-accent)" : color.hex}80` : "none",
                      }}
                    />
                  ))}
                </div>
              ...
              <div className="pt-4 border-t border-border flex justify-end gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg text-xs font-semibold bg-accent hover:bg-accent-hover text-accent-text transition-colors flex items-center gap-1.5"
                >
