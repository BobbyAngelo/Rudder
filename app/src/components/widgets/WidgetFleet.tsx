    ...
    async function loadFleet() {
      try {
        ...
        if (data.cluster) {
          ...
          const mapped = data.cluster.slice(0, 4).map((node: ClusterNode) => {
            let icon = Server;
            if (node.name === "RUDDER") icon = Monitor;
            if (node.name === "KIPP") icon = Database;
            if (node.name === "MURPH" || node.name.includes("CYD")) icon = Radio;
