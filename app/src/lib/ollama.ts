...
// Local Ollama by default. Power users with extra nodes can list
// comma-separated fallbacks in OLLAMA_FALLBACK_URLS — no hosts are
// hardcoded, so nothing on your network is contacted without opting in.
const OLLAMA_HOSTS = [
  process.env.OLLAMA_URL || "http://localhost:11434",
  ...(process.env.OLLAMA_FALLBACK_URLS || "")
    .split(",")
    .map((h) => h.trim())
    .filter(Boolean),
];
...
async function getOllamaHost(): Promise<string> {
  ...
  throw new Error(`No Ollama host reachable (tried: ${OLLAMA_HOSTS.join(", ")}). Is Ollama running?`);
}
