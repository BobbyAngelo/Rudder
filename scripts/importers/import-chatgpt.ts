...
/**
 * Rudder 1.0 — ChatGPT Conversations Import Script
 * Reads OpenAI's conversations.json and imports conversations into journal_entries.
 */

import Database from "better-sqlite3";
import path from "path";
import fs from "fs";
import { readChatGPTExport } from "../../app/src/lib/ingest/chatgpt";
...
const log = (msg: string) => console.log(`[chatgpt] ${msg}`);
const ok = (msg: string) => console.log(`[chatgpt] ✅ ${msg}`);

async function main() {
  console.log("╔═══════════════════════════════════════════╗");
  console.log("║  Rudder 1.0 — ChatGPT Import              ║");
  console.log("╚═══════════════════════════════════════════╝");
  console.log("");

  if (!filePath) {
    console.error("Usage: npx tsx scripts/importers/import-chatgpt.ts <path/to/conversations.json> [--dry-run]");
    process.exit(1);
  }
  ...
  const docs = readChatGPTExport(filePath);
  ...
  const tx = db.transaction(() => {
    for (const doc of docs) {
      ...
      const tags = JSON.stringify(["chatgpt"]);
