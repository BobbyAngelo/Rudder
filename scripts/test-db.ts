...
const POSSIBLE_DB_PATHS = [
  path.join(__dirname, "..", "data", "rudder.db"),
  path.join(__dirname, "..", "..", "data", "rudder.db"),
  path.resolve(process.cwd(), "data", "rudder.db"),
  path.resolve(process.cwd(), "..", "data", "rudder.db")
];
...
if (!dbPath) {
  // If not found, try to create it in the standard location
  dbPath = path.resolve(__dirname, "..", "data", "rudder.db");
  const dataDir = path.dirname(dbPath);
  if (!fs.existsSync(dataDir)) {
    fs.mkdirSync(dataDir, { recursive: true });
  }
  console.log(`[test-db] Database not found. Initializing new test database at: ${dbPath}`);
} else {
  console.log(`[test-db] Found database at: ${dbPath}`);
}
...
for (const table of EXPECTED_TABLES) {
  try {
    ...
    if (info && info.count > 0) {
      ...
      // Check virtual tables (like search_index)
