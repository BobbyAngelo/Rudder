...
async function handleRequest(req: any): Promise<any> {
      ...
      result: {
        tools: [
          ...
          },
          {
            ...
            description: "Retrieve the latest biometric telemetry readings (e.g. HeartRate, StepCount, SleepAnalysis) from the health ledger.",
            inputSchema: {
              type: "object",
              properties: {
                limit: {
          ...
          },
          {
            ...
            description: "Retrieve the latest cross-domain correlation insights generated by the system analysis engines.",
            inputSchema: {
              type: "object",
              properties: {
                limit: {
  ...
  if (method === "tools/call") {
    ...
    try {
      ...
      if (name === "rudder_search_memory") {
      ...
      else if (name === "rudder_get_biometric_state") {
        const limit = Math.min(toolArgs.limit || 10, 50);
        const rows = db.prepare(`
          SELECT type, value, category_value, unit, source, start_date, date
          FROM health_records 
        ...
        if (rows.length === 0) {
          resultText = "No biometric records found in the health ledger.";
        } else {
          resultText = `Latest ${rows.length} biometric records:\n\n` + rows.map(r => {
            const val = r.value !== null ? `${r.value} ${r.unit || ""}` : r.category_value;
            return `💓 [${r.date}] ${r.type}: ${val} (source: ${r.source})`;
          }).join("\n");
        }
      }

      else if (name === "rudder_get_recent_insights") {
        const limit = Math.min(toolArgs.limit || 5, 20);
        const rows = db.prepare(`
          SELECT event_id, when_timestamp, why_insight, gravity_score
          FROM reality_nodes 
        ...
        if (rows.length === 0) {
          resultText = "No correlation insights found in the ledger.";
        } else {
          resultText = `Latest ${rows.length} correlation insights:\n\n` + rows.map(r => {
            return `💡 [${r.when_timestamp}] (Score: ${r.gravity_score}/10)\n  "${r.why_insight}"\n`;
          }).join("\n");
        }
      }
